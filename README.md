...
| with files<br>...<br>:release `ARGS` | 'github' \<owner²/repo²>[@branch²] \<tag²> &#124; ('am'&#124;'forum'&#124;'forums') \<threadid> \<version²> &#124; 'zip' \<file²> &#124; 'updater' \<updaterFile²> \<version²> | Create a new Release on either github, the forums, create a release zip or patch the updater files with the files listed in the lines between `with files` and `:release ...`. The colon is used as that creates an invalid path.                                                                                                                |
